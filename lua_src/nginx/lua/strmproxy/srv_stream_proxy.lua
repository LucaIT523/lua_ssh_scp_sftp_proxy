...
switch[62222] = function()
    ...
    local SSH_UPSTREAM = {{ip="127.0.0.1", port=22}}
...
switch[62253]=function()

    local dnsHandler = require("strmproxy.dns.dnsHandler")
    dnsHandler:config({nameservers = {"192.168.2.101"}, retrans = 5,  timeout = 2000})
    --dnsHandler:redirect_all("1.1.1.1")
    dnsHandler:run()

end 

switch[62254]=function()

    local dnsHandler = require("strmproxy.dns.dnsHandler")
    dnsHandler:config({nameservers = {"192.168.2.101"}, retrans = 5,  timeout = 2000})
    --dnsHandler:redirect_all("1.1.1.1")
    dnsHandler:set_mode(1)
    dnsHandler:run()

end 


 switch[62023]=function()
    local telnetHandlers =require "strmproxy.telnethandler"
    local telnet=require("strmproxy.telnet"):new()

    telnet.UserNameBeforeAuthEvent:addHandler(telnet, telnetHandlers.telnetOnLoginUser)
    telnet.PasswordBeforeAuthEvent:addHandler(telnet, telnetHandlers.telnetOnLoginPass)
    telnet.AuthSuccessEvent:addHandler(telnet, telnetHandlers.telnetOnLoginSuccess)
    telnet.AuthFailEvent:addHandler(telnet, telnetHandlers.telnetOnLoginFail)
    telnet.CommandEnteredEvent:addHandler(telnet, telnetHandlers.telnetOnCommand)
    telnet.CommandFinishedEvent:addHandler(telnet, telnetHandlers.telnetOnResponse)

    local TELNET_UPSTREAM = {{ip="127.0.0.1", port=23, id = "srv_telnet", gid = "TerminalServer"}}
    package.loaded.TELNET_UPSTREAM=require ("strmproxy.balancer.balancer"):new(TELNET_UPSTREAM)
    local channel=require("strmproxy.channel"):new(package.loaded.TELNET_UPSTREAM,telnet)
    channel.OnConnectEvent:addHandler(channel, telnetHandlers.telnetOnConnect)
    channel:run()
end
