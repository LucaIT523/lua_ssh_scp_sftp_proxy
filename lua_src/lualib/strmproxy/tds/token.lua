...
local datetime = require "strmproxy.utils.datetime"
...
----------------------------row parser------------------------------
local binRow=function(bytes,pos,length) local tmp,pos=string.unpack("c"..length,bytes,pos) return tmp:hex(),pos end

local ntextRow=function(bytes,pos,length) local result,pos= string.unpack("c"..length,bytes,pos) return unicode.utf16to8(result),pos end

local textRow=function(bytes,pos,length) local result,pos= string.unpack("c"..length,bytes,pos) return result,pos end
--signless int
local intRow=function(bytes,pos,length) return string.unpack("<I"..length,bytes,pos) end
...
local decimalRow=function(bytes, pos,length,c)
    ...
    logger.dbg("Unhandled lengthgth (%d) for DECIMALNTYPE", length)
...
local partial=function(bytes,pos,length,c,realParser) 
    ...
    local result={} 
    local chuckLen,pos=string.unpack("<I4",bytes,pos)
    local s,pos=realParser(bytes,pos,chuckLen) 
    while s~=""  do 
        table.insert(result,s) 
        chuckLen,pos=string.unpack("<I4",bytes,pos)
...
-----------------------length parser------------------------------------
local ltsLen=	function(c,bytes,pos) assert(c.lts) if c.lts==0xffff then return string.unpack("<I8",bytes,pos) else return string.unpack("<I2",bytes,pos) end end
...
local DataTypes ={
  ...
  NTEXTTYPE           = {code=0x63, lenBytes=4,    headerParser=textXHeader,   parser=binRow,          desc="NTEXTTYPE",                 },
...
local ssvarRow=function(bytes,pos,length,c)
    local prop={}
    local baseType,propLen,pos=string.unpack("BB",bytes,pos)
...
_M.Token={
    parse=function(self,bytes,pos)
        local err
        self.type,pos=string.unpack("B",bytes,pos)
        if TokenType[self.type] then 
            -- print(TokenType[self.type].desc.." is parsing") 
            logger.dbg(TokenType[self.type].desc.." is parsing") 
        else
            -- print(string.format("%02X",self.type).." is parsing") 
            logger.dbg(string.format("%02X",self.type).." is parsing") 
        end
        -- print(self.length)
        ...
        if TokenType[self.type] then 
            ...
            logger.dbg(TokenType[self.type].desc.." finished") 
        else
            -- print(string.format("%02X",self.type).." finished") 
            logger.dbg(string.format("%02X",self.type).." finished") 
        end
        return pos
    end,
    parseData=function(self,bytes,pos)
        -- logger.dbg("\027[33m> [parseData] bytes      :\027[0m ("..#bytes..") ", bytes)
        --for those packet we don't care just skip it
        if (not self.length) and (not self.count) then
            logger.log(logger.ERR,self.type..string.format("(%02X)",self.type).." token can not be parsed")
            return nil
        end
        return pos+(self.length or 0)
    end,
    ...
    parseLength=function(self,bytes,pos)
        ...
        if self.type==TokenType.ReturnStatus.code then 
            self.length=4 
        elseif self.type==TokenType.DoneInProc.code then 
            self.length=12 
        elseif self.type==TokenType.DoneProc.code then 
            self.length=12 
        elseif bit.band(self.type,0x30)==0x10 then
            self.length=0
        ...
        elseif bit.band(self.type,0x30)==0x20 then
            self.length,pos=string.unpack("<I2",bytes,pos)
        --count type, length not available 0000 0000
        elseif bit.band(self.type,0x30)==0x00 then
            self.count,pos=string.unpack("<I2",bytes,pos)
...
_M.ColumnMetaDataToken = {
    ...
    parseData=function(self,bytes,pos)
        for i=1,self.count,1 do
            ...
            local nextPos, bytesLength = 0, #bytes
            while DataTypes[column.colType] == nil and pos <= bytesLength do
                column.userType, column.flags, column.colType, nextPos = string.unpack("<I4I2B", bytes, pos)
                pos = pos + 1
            end
            pos = nextPos

            -- logger.dbg("\r\n\027[32m@parseData DataTypes[column.colType]:\027[0m " .. tableUtils.printTableF(DataTypes[column.colType]))
            -- logger.dbg("\r\n\027[32m@parseData column:\027[0m " .. tableUtils.printTableF(column))
            -- logger.dbg("\027[32m@parseData> pos :\027[0m ", pos)
            --print(column.colType)
            if DataTypes[column.colType] and DataTypes[column.colType].headerParser then
...
_M.LoginAckToken = {
    ...
    parseData=function(self,bytes,pos)
        self.interface,pos=string.unpack("B",bytes,pos)
        self.TDSVersion,pos=string.unpack(">c4",bytes,pos)
        local progNameLen,pos=string.unpack("B",bytes,pos)
        self.progName,pos=unicode.utf16to8(string.unpack("c"..progNameLen*2,bytes,pos)),pos+progNameLen*2
        local major, minor, build ,pos=string.unpack(">BBI2",bytes,pos)
                ...
                if bit.band(nullBytes[math.ceil(i/8)],bit.lshift(maskbit,(i%8)==0 and 7 or (i%8)-1))==0 then
                    ...
                    if dtype then
                        ...
                        if not length then
                            ...
                            length,pos=string.unpack("I"..lenBytes,bytes,pos)
                        ...
                        length,pos=string.unpack("B",bytes,pos)
                        value,pos=string.unpack("c"..lenBytes,bytes,pos)
                    end
                else
                    value="NULL"
                end
...
_M.ErrorToken = {
    ...
    parseData=function(self,bytes,pos)
        self.number,pos=string.unpack("<I4",bytes,pos)
        self.state,pos=string.unpack("B",bytes,pos)
        self.class,pos=string.unpack("B",bytes,pos)
        local msgLen,pos=string.unpack("I2",bytes,pos)
        self.message,pos=unicode.utf16to8(string.unpack("c"..msgLen*2,bytes,pos)),pos+msgLen*2
        local serverNameLen,pos=string.unpack("B",bytes,pos)
        self.serverName,pos=unicode.utf16to8(string.unpack("c"..serverNameLen*2,bytes,pos)),pos+serverNameLen*2
        local procNameLen,pos=string.unpack("B",bytes,pos)
        self.procName,pos=unicode.utf16to8(string.unpack("c"..procNameLen*2,bytes,pos)),pos+procNameLen*2
        self.lineNo,pos=string.unpack("<I4",bytes,pos)
        return pos,nil
    end,
    pack=function(self)
        local buf={
            string.pack("<I4BB",self.number,self.state,self.class),
            string.pack("<I2",self.message:len()),
            unicode.utf8to16(self.message),
            string.pack("B",self.serverName:len()),
            unicode.utf8to16(self.serverName),
            string.pack("B",self.procName:len()),
            unicode.utf8to16(self.procName),
            string.pack("<I4",self.lineNo)
        }
        local tmp=table.concat(buf)
        return string.pack("B",self.type)..string.pack("<I2",#tmp)..tmp
    end,
    tostring=function(self)
