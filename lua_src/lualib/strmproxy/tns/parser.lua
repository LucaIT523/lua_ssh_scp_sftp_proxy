...
--tns protocol parser 
local P=require "strmproxy.tns.tnsPackets"
local K=P.getKey
...
local conf={
	...
	{key=K({dataId=8,req=K({callId=0x73})}), parser=P.AuthResponse,   eventName="AuthResponseEvent"},
...
local keyG=function(allBytes,pos,options,request)
	...
	if dataId==P.DataID.USER_OCI_FUNC.code or dataId==P.DataID.PIGGYBACK_FUNC.code then 
		callId=allBytes:byte(8+pktChkLen+hdrChkLen) 
	end
