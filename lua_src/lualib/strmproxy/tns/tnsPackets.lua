...
cjson = ok and cjson or require("strmproxy.utils.json")
...
local parserUtil=require ("strmproxy.parser")
local _M={}
----------------------build parser---------------------------
...
tableUtils.addIndex(packetType,"code")
...
--build index for code
tableUtils.addIndex(dataId,"code")
...
local function parseFemagic(is64Bit,bytes,pos) return string.unpack(("c"..(is64Bit and 8 or 1)),bytes,pos) end
...
_M.Packet={
	...
	parseHeader=function(self,headerBytes,pos)
		self._length,pos=string.unpack(">I2",headerBytes,pos)
		if self.options:pktChk() then self._packetCheckSum,pos=string.unpack(">I2",headerBytes,pos) end
		self._code,pos=string.unpack("B",headerBytes,pos)
		self._reserved,pos=string.unpack("B",headerBytes,pos)
		if self.options:hdrChk() then self._headerCheckSum,pos=string.unpack(">I2",headerBytes,pos) end
		if self._code==packetType.DATA.code then self._dataFlag,pos=string.unpack(">I2",headerBytes,pos) end
		return pos
	end,
	packHeader=function(self,payloadLen)
		local rs={
			string.pack(">I2",payloadLen+4
				+(self.options:pktChk() and 2 or 0)
				+(self.options:hdrChk() and 2 or 0)
				+(self._code==packetType.DATA.code and 2 or 0)),
			self.options:pktChk() and string.pack(">I2",self._packetCheckSum) or "",
			string.pack("B",self._code),
			string.pack("B",self._reserved),
			self.options:hdrChk() and string.pack(">I2",self._headerCheckSum) or "",
			self._code==packetType.DATA.code and string.pack(">I2",self._dataFlag) or "",
		}
	...
	pack=function(self)
		...
		logger.dbgWithTitle("packing",result:hex32F())
...
_M.Connect={
	...
	parsePayload=function(self,bytes,pos)
		self._tnsVersion,pos=string.unpack(">I2",bytes,pos)
		self._compatibleVersion,pos=string.unpack(">I2",bytes,pos)
		self._serviceOption,pos=string.unpack(">I2",bytes,pos)
		--header checksum
		self._sessionDataUnit,pos=string.unpack(">I2",bytes,pos)
		self._maxTranmitDataUnit,pos=string.unpack(">I2",bytes,pos)
		self._netOptions,pos=string.unpack(">I2",bytes,pos)
		self._line,pos=string.unpack(">I2",bytes,pos)
		self._valueOf1,pos=string.unpack(">I2",bytes,pos)
		local connectDataLength,pos=string.unpack(">I2",bytes,pos)
		self._connectDataOffset,pos=string.unpack(">I2",bytes,pos)
		self._maxRecvConnData,pos=string.unpack(">I4",bytes,pos)
		self._flag1,pos=string.unpack("B",bytes,pos)
		self._flag2,pos=string.unpack("B",bytes,pos)
		self._idontcare,pos=string,unpack(("c"..self._connectDataOffset+1-pos),bytes,pos)
		self._connStr=string.unpack(("c"..connectDataLength),bytes,self._connectDataOffset+1)
		return self
	end,
	packPayload=function(self)
		local result={
			string.pack(">I2",self._tnsVersion),
			string.pack(">I2",self._compatibleVersion),
			string.pack(">I2",self._serviceOption),
			string.pack(">I2",self._sessionDataUnit),
			string.pack(">I2",self._maxTranmitDataUnit),
			string.pack(">I2",self._netOptions),
			string.pack(">I2",self._line),
			string.pack(">I2",self._valueOf1),
			string.pack(">I2",self._connStr:len()),
			string.pack(">I2",self._connectDataOffset),
			string.pack(">I4",self._maxRecvConnData),
			string.pack("B",self._flag1),
			string.pack("B",self._flag2),
			self._idontcare,
			self._connStr,
		}
...
_M.Accept={
	...
	parsePayload=function(self,bytes,pos)
		...
		self._maxTranmitDataUnit,pos=string.unpack(">I2",bytes,pos)
		self._valueOf1,pos=string.unpack(">I2",bytes,pos)
		local dataLength,dataOff,pos=string.unpack(">I2I2",bytes,pos)
		self._flag1,pos=string.unpack("B",bytes,pos)
		self._flag2,pos=string.unpack("B",bytes,pos)
	end
}
...
local unpackNumber = function( bytes, pos )
	local len, pos = string.unpack("<B", bytes, pos)
	if (len == 0) then
		return 0, pos
	end
	local value, pos = string.unpack("<I" .. len, bytes, pos)
	return value, pos
end

local parseKVPEntry = function( data, pos, useByte)
	if (useByte == nil) then
		useByte = false
	end
	...
	if (useByte) then
		value_len, pos = unpackNumber(data, pos)
	else
...
local parseKVP = function( data, pos, useByte)
	local key, value, flags
	pos, key   = parseKVPEntry( data, pos, useByte )
	pos, value = parseKVPEntry( data, pos, useByte )
	if (useByte) then
		flags, pos = unpackNumber(data, pos)
	else
...
_M.SessionRequest={
	...
	parsePayload=function(self,bytes,pos)
---[[
		self._dataId,pos=string.unpack("B",bytes,pos)
		assert(self._dataId == self:getDataId(),self._dataId)
		self._callId,pos=string.unpack("B",bytes,pos)
		assert(self._callId == self:getCallId(),self._callId )
		self._seq,pos=string.unpack("B",bytes,pos)
		local userNamePos,countPos, paramCount
		...
		if(self._is64Bit) then
			...
			for i=1, paramCount do
				local item,k={}, nil
				if (self._use4Byte) then
					pos, k, item.v, item.flag = parseKVP( bytes, pos )
				else
					pos, k, item.v, item.flag = parseKVP( bytes, pos, true)
				end
				params[k] = item
			end
			self._params=params
		else
			self._use4Byte=bytes:byte(8 + 4 + 2 + 4)==0x00 and true or false
			print(string.format( "use4Byte = %s %02x", self._use4Byte, bytes:byte(8 + 4 + 2 + 4)))
			
			if (self._use4Byte) then
				local userNameLen, pos = string.unpack("<I4", bytes, pos)
				self._loginmode, pos = string.unpack("<I4", bytes, pos)
				paramCount, pos = string.unpack("<I4", bytes, pos + 1)
				local realUserNameLen = string.unpack("B", bytes, pos + 2)
				self._username, pos = string.unpack("s1", bytes, pos + 2)
				local params = orderTable:new()
				for i=1, paramCount do
					local item,k={}, nil
					pos, k, item.v, item.flag = parseKVP( bytes, pos )
				...
				self._unknown=bytes:sub(pos,pos+8)
				local userNameLen, pos = unpackNumber(bytes, pos)
				self._loginmode, pos = unpackNumber(bytes, pos)
				paramCount, pos = unpackNumber(bytes, pos + 1)
...
--]]
	...
	packPayload=function(self)
		local rs={
			string.pack("BBB",self._dataId,self._callId,self._seq),
			self._loginmode,
			string.pack("s1",self._username),
		}
...
_M.SessionResponse={
	...
	parsePayload=function(self,bytes,pos)
		self._dataId,pos=string.unpack("B",bytes,pos)
		self._use4Byte = bytes:byte(pos) ~= 0x01 and true or false
		assert(self._dataId == self:getDataId(),self._dataId)
		local kvps = {}
		local c
		if (self._use4Byte) then
			c,pos =string.unpack("<I2",bytes, pos)
		else
			c,pos =unpackNumber(bytes, pos)
		end
		self._params=orderTable:new()
		for i=1, c do
			local item,k={}, nil
			
			pos, k, item.v, item.flag = parseKVP( bytes, pos, self._use4Byte ~= true)
			self._params[k] = item
		end
	...
	packPayload=function(self)
		local rs={
			string.pack("B<I2",self._dataId,#(self._params))
		}
		for i,item in ipairs(self._params) do
...
_M.AuthResponse={
	...
	parsePayload=function(self,bytes,pos)
		self._dataId,pos=string.unpack("B",bytes,pos)
		self._use4Byte = bytes:byte(pos) ~= 0x01 and true or false
		print(string.format( "use4Byte= %s %s", self._use4Byte, bytes:byte(pos) ))
		assert(self._dataId == self:getDataId(),self._dataId)
		local kvps = {}
		local c
		if (self._use4Byte) then
...
_M.SetProtocolRequest={
	...
	parsePayload=function(self,bytes,pos)
		self._dataId,pos=string.unpack("B",bytes,pos)
		assert(self._dataId == self:getDataId(),self._dataId)
		local acceptedVersion,pos=string.unpack("z",bytes,pos)
		self._acceptedVersion={acceptedVersion:byte(1,#acceptedVersion)}
		self._clientPlatform,pos=string.unpack("z",bytes,pos)
		return self,nil
	end
}
...
_M.Piggyback={
	...
	desc="Piggyback",
	parsePayload=function(self,bytes,pos)  
		self._piggyDataId,pos=string.unpack("B",bytes,pos)
		assert(self._piggyDataId == self:getDataId(),self._dataId)
		self._piggyCallId,pos=string.unpack("B",bytes,pos)
		self._piggySeq,pos=string.unpack("B",bytes,pos)
		local cHeaderPos
		...
		self._piggyHeader,pos=string.unpack(("c"..cHeaderPos-pos),bytes,pos)
		...
		local entry=require("strmproxy.tns.parser"):new().C2PParser:getParser(key)
		self.__key=key
		if entry then
	...
	packPayload=function(self)
		local rs={
			string.pack("BBB",self._piggyDataId,self._piggyCallId,self._piggySeq),
			self._piggyHeader,
		}
		local key=getKey({callId=self._realCallId,dataId=self._realDataId,code=self._code})
		local entry=require("strmproxy.tns.parser"):new().C2PParser:getParser(key)
...
_M.VersionResponse={
	...
	parsePayload=function(self,bytes,pos)
		...
		local minor
		self._fix,self._sub,minor,self._major,pos=string.unpack("BBBB",bytes,pos)
...
_M.SQLRequest={
	...
	getSqlTotalLen=function(self) 
		local sqlLen=string.unpack(("<I"..self._sqlTotalLen:len()),self._sqlTotalLen) 
	...
	parsePayload=function(self,bytes,pos)
		...
		assert(self._callId == self:getCallId(),self._callId )
		self._seq,pos=string.unpack("B",bytes,pos)
		self._useByte = string.unpack("B", bytes, pos + 4) == 0x01 and true or false
		local unknowLen=8 local sqlLen=2 local unknowLen2=2 local fixUnknownLen=152
		if self._useByte then
			unknowLen = 5
			sqlLen = string.unpack("B", bytes, pos + 5)
			unknowLen2 = 3
		end
		if self.options.platform=="Java_TTC-8.2.0" then unknowLen=5 sqlLen=1 unknowLen2=1 end
		self._unknown,pos=string.unpack(("c"..unknowLen),bytes,pos)
		self._feMagic,pos=parseFemagic(self.options.is64Bit,bytes,pos)
		self._sqlTotalLen,pos=string.unpack(("c"..sqlLen),bytes,pos)
		self._unknown2,pos=string.unpack(("c"..unknowLen2),bytes,pos)
		...
		if self._useByte then fixUnknownLen = 25 fixUnknownLen = 25 end
		self._fixUnknown,pos=string.unpack(("c"..fixUnknownLen),bytes,pos)
		local len_len=1
		--client 19_6
		...
		if(commandLen==0xfe) then
			local i=pos+1
			while(string.unpack(("I"..len_len),bytes,i)~=0) do
				local c
...
--print(tableUtils.printTableF(_M.parser))
...
function _M.unitTest.connectTest()
	...
	local packet=require("strmproxy.tns.parser"):new().C2PParser:parse(bytes)
...
function _M.unitTest.acceptTest()
	...
	local packet=require("strmproxy.tns.parser"):new().S2PParser:parse(bytes)
...
function _M.unitTest.sessionResponseTest()
	...
	local packet=require("strmproxy.tns.parser"):new().S2PParser:parse(bytes,nil,nil,nil,"callId"..0x76)
...
function _M.unitTest.setProtocolRequestTest()
	...
	local packet=require("strmproxy.tns.parser"):new().C2PParser:parse(bytes)
	assert(#packet._acceptedVersion==6,tableUtils.printTableF(packet._acceptedVersion))
...
function _M.unitTest.SQLRequestTest()
	...
	packet:pack()
	bytes=packet.allBytes
	packet=require("strmproxy.tns.parser"):new().C2PParser:parse(bytes)
...
function _M.unitTest.PiggybackTest()
	...
	local packet=require("strmproxy.tns.parser"):new().C2PParser:parse(bytes,nil,nil,opt)
	local oldbytes=bytes 
...
function _M.unitTest.versionTest()
	...
	local packet=require("strmproxy.tns.parser"):new().S2PParser:parse(bytes,nil,nil,nil,"callId"..0x3b)
