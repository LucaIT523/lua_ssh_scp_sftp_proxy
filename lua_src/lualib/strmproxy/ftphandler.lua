...
local function ftpOnConnect(context,eventSource, connInfo)
    local rs={
...
local function ftpOnLogin(context,eventSource, cred, session)
    local log
...
local function ftpOnLoginSuccess(context,eventSource, username, session)
    local log
    log = format("Login OK (user : %s)", session.username)
    logger.ntc(format("[StrmProxy] FTP %s", log))
end

local function ftpOnLoginFail(context,eventSource, username, session)
    local log
    log = format("Login Fail (user : %s)", session.username)
    logger.ntc(format("[StrmProxy] FTP %s", log))
end

local function ftpOnCommand(context,eventSource, command, session)
    local log
    local res = {}
...
local function ftpOnResponse(context,eventSource, response, session)
    local log
    local res = {}
...
local function ftpOnFTPData(context,eventSource, packet, session)
    local log
    local res = {}
...
_M.ftpOnConnect = ftpOnConnect
_M.ftpOnLogin = ftpOnLogin
_M.ftpOnLoginSuccess = ftpOnLoginSuccess
_M.ftpOnLoginFail = ftpOnLoginFail
_M.ftpOnCommand = ftpOnCommand
_M.ftpOnResponse = ftpOnResponse
_M.ftpOnFTPData = ftpOnFTPData
