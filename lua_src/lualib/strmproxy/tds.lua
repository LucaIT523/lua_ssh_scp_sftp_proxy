...
local tdsPacket=require "strmproxy.tds.tdsPackets"
...
--	if options.disableSSL~=nil then o.disableSSL=options.disableSSL end
	...
	local tdsParser=require ("strmproxy.tds.parser"):new(o.catchReply)
    ...
    o.C2PParser.events.RemoteProcedureCall:addHandler(o,_M.RemoteProcedureCallHandler)
    ...
    o.S2PParser.events.RPCResponse:addHandler(o,_M.SQLResponseHandler)
...
function _M:RemoteProcedureCallHandler(src,p)
    self.ctx.rpc = p.rpc
    command = string.format("%s %s %s", p.rpc.procedure, p.rpc.paramName, p.rpc.paramValue)
...
function _M:PreloginHandler(src,p)
    logger.dbg(">[C2PParser.events.Prelogin:PreloginHandler] username: ", p.username and p.username or "nil")
...
function _M:Login7Handler(src,p)
    ...
    if self.BeforeAuthEvent:hasHandler() then
        logger.dbg(">[BeforeAuthEvent:Login7Handler] username: ", p.username and p.username or "nil")
...
function _M:LoginResponseHandler(src,p)
    if p.success then
		...
		if self.AuthFailEvent:hasHandler() then
			...
			self.AuthFailEvent:trigger(self.ctx.username,{message="["..p.errNo.."]"..p.message},self.ctx)
		end
	end
end
...
function _M:SQLResponseHandler(src,p)
    if self.CommandFinishedEvent:hasHandler() then
        ...
        reply=reply
        -- logger.dbg(">[SQLResponseHandler] reply:\r\n" .. reply)
...
function _M.processDownRequest(self)
    ...
    -- Set whether to record incoming data -------------------------------------
    -- self.S2PParser.skip_parse = true
    ------------------------------------- Set whether to record incoming data --
