...
local function unpackDouble (e,l,s)
    local rs=s:sub(1,l)
    -- if little endiad, reverse string
    if e=="<" then rs=rs:reverse() end
    -- get byte from 1st to 8th
    ...
    -- Check for denormalized numbers (when both exponent and mantissa are 0)
    local n
...
local function getLen(t,l)
    local _t={
        ["B"]=1,        -- unsigned 1-byte integer
        ["b"]=1,        -- signed 1-byte integer
        ["H"]=2,        -- unsigned 2-byte integer
        ["h"]=2,        -- signed 2-byte integer
        ["d"]=8,        -- double (8 bytes)
        ["f"]=4,        -- float (4 bytes)
        ["s"]=1,        -- string (1 byte per character, likely used for fixed-length strings)
        ["I"]=_INTLEN,  -- unsigned integer, length determined by _INTLEN
        ["i"]=_INTLEN   -- signed integer, length determined by _INTLEN
    }
...
function string.pack(fmt,...)
    local arg={...}
    -- logger.dbg("\r\n@pack arg:" .. tableUtils.printTableF(arg))
    ...
    for w,e,t,l in fmt:gmatch("(([<>=]?)([bBhH1LjJTiIfdnczsxX])([%d]*))") do
        -- string.pack( ">BI2I2", optionType, offset, optionLength )
        -- format,       'w' corresponds to the entire matched string '((([<>=]?)([bBhH1LjJTiIfdnczsxX])([%d])))'.
        -- optionType,   'e' corresponds to the part that matches the '[<>=]?' ENDIAN pattern.
        -- offset,       't' corresponds to the part that matches the '[bBhH1LjJTiIfdnczsxX]' pattern.
        -- optionLength, 'l' corresponds to the part that matches the '[0-9]*' pattern.
        -- logger.dbg("\027[7;42m ", e, t, l, " / ", fmt, ", arg["..i.."] (", type(arg[i]), "):\027[0m ", arg[i] or "nil")
    ...
    -- logger.dbg("\027[7;42m> rs (", type(rs), "):\027[0m ", rs)
    return rs
end
...
function string.unpack(fmt,value,pos)
    ...
    local nativeEndian=_DEFAULT_ENDIAN
    for w,e,t,l in fmt:gmatch("(([<>=]?)([bBhH1LjJTiIfdnczsxX])([%d]*))") do
        -- variable 'w' corresponds to the entire matched string '((([<>=]?)([bBhH1LjJTiIfdnczsxX])([%d])))'.
        -- variable 'e' corresponds to the part that matches the '[<>=]?' pattern.
        -- variable 't' corresponds to the part that matches the '[bBhH1LjJTiIfdnczsxX]' pattern.
        -- variable 'l' corresponds to the part that matches the '[0-9]*' pattern.
        -- logger.dbg("\027[7;42m ", e, t, l, " / ", fmt, ", arg["..i.."] (", type(arg[i]), "):\027[0m ", arg[i] or "nil")
        l=tonumber(l)
        if(e:len()~=0) then nativeEndian=e end
    ...
    table.insert(rs,i)
    -- logger.dbg("\r\n\027[32m@unpack rs:\027[0m " .. tableUtils.printTableF(rs))
    return unpack(rs)
end
