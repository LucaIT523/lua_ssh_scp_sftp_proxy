...
local function addHandler(self,context,...)
    local handlers={...}
    if #handlers == 0 then
        logger.dbg("no handler was added to event")
        return logger.err("no handler was added to event")
    end
...
function _M:new(source,name)
    ...
    o.trigger = function(self,...)
                    logger.dbgWithTitle(string.format("Event [%s] by new (chain len: %d)", self.name, #(self.chain)))
                    -- logger.dbg("> chain len: " .. #(self.chain) .. " (" .. self.name .. ")")
                    local args={...}
...
function _M:newReturnEvent(source,name)
    ...
    o.trigger = function(self,...)
                    logger.dbgWithTitle(string.format("Event [%s] by newReturnEvent (chain len: %d)", self.name, #(self.chain)))
                    -- logger.dbg("> chain len: " .. #(self.chain) .. " (" .. self.name .. ")")
                    local args={...}
                    for k,v in ipairs(self.chain) do
