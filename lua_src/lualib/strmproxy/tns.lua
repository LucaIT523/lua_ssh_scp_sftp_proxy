...
local tnsPackets=require "strmproxy.tns.tnsPackets"
local tableUtils=require "strmproxy.utils.tableUtils"
local crypt= require "strmproxy.tns.crypt"
...
function _M.new(self,options)	
    ...
    o.ctx={}
    local tnsParser=require ("strmproxy.tns.parser"):new()
    ...
    o.S2PParser.events.AuthResponseEvent:setHandler(o,_M.AuthResponseHandler)
...
function _M:PiggbackHandler(src,p)
    ...
    if entry and entry.event and entry.event:hasHandler() then
        if self.CommandEnteredEvent:hasHandler() then
            logger.dbg("\027[1;31m>[PiggbackHandler] command: ", #command, " ", command, "\027[0m")
            self.CommandEnteredEvent:trigger(command, self.ctx)
        end
