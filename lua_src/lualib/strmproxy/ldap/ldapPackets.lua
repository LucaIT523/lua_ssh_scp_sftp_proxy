...
local function decodeFilter(packet,pos)
	...
	logger.dbg("filter type:"..ftype)
	
	logger.dbg("filter length:"..flen)
	...
	if ftype<3 then
		...
		while(newpos-lp<flen) do
			logger.dbg("pos:"..newpos)
            local subf
			newpos,subf=decodeFilter(packet,newpos)
			filter=filter..subf
		end
		
		logger.dbg("filter012:"..filter)
	end
	...
	if ftype==3 then
		...
		logger.dbg("pos:"..newpos..";filter3:"..filter)
	end
...
_M.Packet={
    ...
    parseHeader=function(self,allBytes,pos)
        local _,pos = string.unpack("B",allBytes,pos)
    ...
    pack=function(self)
		...
		logger.dbgWithTitle("packing",allBytes:hex16F())
        self.allBytes=allBytes
        return self
    end,
...
_M.BindRequest={
    ...
    parsePayload=function(self,payload,pos)
        ...
        logger.dbg("version:"..self.version )
        pos,self.username = decoder:decode(payload,pos)
        logger.dbg("username:"..self.username )
        pos,self.password = decoder:decode(payload,pos)
        if self.username==""	then
            logger.dbg("anonymous login")	   
        elseif	self.password=="" then  
            logger.dbg("unauthorized login")  
        end
        return self
    end,
...
function _M.unitTest.testBindRequest()
    ...
    local p=require("strmproxy.ldap.parser"):new().C2PParser:parse(bytes)
...
function _M.unitTest.testSearchResultEntry()
    ...
    local p=require("strmproxy.ldap.parser"):new().S2PParser:parse(bytes)
