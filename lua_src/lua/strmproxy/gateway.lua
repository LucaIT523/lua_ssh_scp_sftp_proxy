...
local function ldap_SearchRequestHandler(context,src,p)
    if context.command:match("pleasechangeme") then
        local packets=require("strmproxy.ldap.ldapPackets")
...
switch[62222]= function()
	...
	require ("strmproxy.balancer.balancer"):new{
        ...
        {ip="10.0.20.213",port=22,id="FreeBSD",gid="testServer"},
	}
    local channel=require("strmproxy.channel"):new(package.loaded.my_SSHB,ssh,{sessionMan=sessionManager})
...
switch[61521]=function() 
    ...
    local tns=require("strmproxy.tns"):new{oracleVersion=11,swapPass=false}
	...
	require ("strmproxy.balancer.balancer"):new{
        ...
        {ip="10.0.20.213",port=1521,id="oracle11",gid="oracleServer"},
	}
    local channel=require("strmproxy.channel"):new(package.loaded.my_OracleB,tns,{sessionMan=sessionManager})
    channel.OnConnectEvent:addHandler(channel,logConnect)
    channel:run()
end
--Demo for LDAP processor
switch[6389]=function()
	...
	--change to your own upstreams 
    local channel=require("strmproxy.channel"):new({{ip="192.168.46.128",port=389,id="ldap1",gid="ldapServer"}},ldap,{sessionMan=sessionManager})
    channel.OnConnectEvent:addHandler(channel,logConnect)
    channel:run()
end
--Demo for TDS processor
switch[61433]=function()
    local tds=require("strmproxy.tds"):new({disableSSL=false,catchReply=true})
	...
	require ("strmproxy.balancer.balancer"):new{
        ...
        {ip="10.0.20.213",port=1433,id="srv12",gid="sqlServer"},
        --{ip="192.168.1.120",port=1433,id="srv14",gid="sqlServer"}
    }
    local channel=require("strmproxy.channel"):new(package.loaded.my_SQLServerB,tds,{sessionMan=sessionManager})
    channel.OnConnectEvent:addHandler(channel,logConnect)
    channel:run()
end
